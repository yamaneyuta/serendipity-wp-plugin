...
class ContentIoHook {
	...
	public function restPreparePostFilter( \WP_REST_Response $response, \WP_Post $post, \WP_REST_Request $request ): \WP_REST_Response {
		...
		$paid_content = $this->post_repository->get( new PostId( $post->ID ) )->paidContent();
	...
	public function wpInsertPostDataFilter( array $data, array $postarr ): array {
		...
		if ( $is_revision_restoring ) {
			...
			$paid_content = $this->post_repository->get( new PostId( $revision ) )->paidContent();   // リビジョンの有料部分を取得
			if ( ! is_null( $paid_content ) ) {
	...
	public function savePostFilter( int $post_id, \WP_Post $_ ): void {
		...
		$post            = $post_repository->get( new PostId( $post_id ) );
...
class WidgetContentBuilder {
	...
	public function build( int $post_id ): string {
		$post_data  = $this->post_repository->get( new PostId( $post_id ) );
