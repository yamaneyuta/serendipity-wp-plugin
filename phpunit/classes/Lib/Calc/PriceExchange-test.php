...
class PriceExchangeTest extends IntegrationTestBase {
	...
	public function setUp(): void {
		...
		$token_table->insert( ChainID::ETH_MAINNET, new Address( '0x0D8775F648430679A709E98d2b0Cb6250d2887EF' ), 'BAT', 18 );
		$token_table->insert( ChainID::ETH_MAINNET, new Address( '0x514910771AF9Ca656af840dff83E8264EcF986CA' ), 'LINK', 18 );
		$token_table->insert( ChainID::ETH_MAINNET, new Address( '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48' ), 'USDC', 6 );
		...
		$this->rate_data_stub->method( 'get' )->willReturnCallback(
			function ( SymbolPair $symbol_pair ) {
				...
				if ( $base === 'ETH' && $quote === 'USD' ) {
					...
					return new Rate( $symbol_pair, Hex::from( new BigInteger( '200000000000', 10 ) ), 8 );
				} elseif ( $base === 'JPY' && $quote === 'USD' ) {
					// 1JPY=0.01USDとする。decimalsは本番環境と同じ8桁
					return new Rate( $symbol_pair, Hex::from( new BigInteger( '1000000', 10 ) ), 8 );
				} elseif ( $base === 'USDC' && $quote === 'USD' ) {
					// 1USDC=1USDとする。decimalsは本番環境と同じ8桁
					return new Rate( $symbol_pair, Hex::from( new BigInteger( '100000000', 10 ) ), 8 );
				} elseif ( $base === 'LINK' && $quote === 'ETH' ) {
					// 1LINK=0.005ETH(=10USD)とする。decimalsは本番環境と同じ18桁
					return new Rate( $symbol_pair, Hex::from( new BigInteger( '5000000000000000', 10 ) ), 18 );
				} elseif ( $base === 'BAT' && $quote === 'ETH' ) {
					// 1BAT=0.00005ETH(=0.1USD)とする。decimalsは本番環境と同じ18桁
					return new Rate( $symbol_pair, Hex::from( new BigInteger( '50000000000000', 10 ) ), 18 );
				}
	...
	public function convertZeroPrice(): void {
		...
		// ASSERT
		$this->assertTrue( Hex::isZero( $ret->amountHex() ) );
	...
	public function convertBATtoETH(): void {
		// ARRANGE
		$price = new Price( Hex::from( new BigInteger( '2000', 10 ) ), 0, 'BAT' );  // 0.1ETHに相当する2000BAT
	...
	public function convertUSDtoETH(): void {
		// ARRANGE
		$price = new Price( Hex::from( new BigInteger( '20000', 10 ) ), 2, 'USD' );  // 200.00USD(=0.1ETH)
	...
	public function convertJPYtoETH(): void {
		// ARRANGE
		$price = new Price( Hex::from( new BigInteger( '100000', 10 ) ), 0, 'JPY' );  // 100000JPY=1000USD=0.5ETH
	...
	public function convertLINKtoBAT(): void {
		// ARRANGE
		$price = new Price( Hex::from( new BigInteger( '1', 10 ) ), 0, 'LINK' );  // 1LINK=100BAT
	...
	public function convertLINKtoUSDC(): void {
		// ARRANGE
		$price = new Price( Hex::from( new BigInteger( '1', 10 ) ), 0, 'LINK' );  // 1LINK=10USDC
	...
	public function convertUSDCtoLINK(): void {
		// ARRANGE
		$price = new Price( Hex::from( new BigInteger( '1', 10 ) ), 0, 'USDC' );  // 1USDC=0.1LINK
